...
def transaction_delete(request, transaction_id):
    """Suppression directe d'une transaction (sans template)"""

    user = request.user

    # Vérification des droits
    if not user.has_finance_access():
        messages.warning(request, "Vous n'avez pas les droits pour supprimer cette transaction.")
        return redirect('finance:transaction_list')
    ...
    messages.success(
        request,
        f"Transaction {transaction.transaction_id} supprimée avec succès."
    )
