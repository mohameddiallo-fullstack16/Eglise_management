...
def dashboard_home(request):
    ...
    for event in context['upcoming_events']:
        event.days_until = (event.start_date - today).days
...
def member_add(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            email = member.email  # peut être vide
            ...
            if email:
                ...
                if User.objects.filter(email=email).exists():
                    messages.warning(
                        request,
                        f"Un compte utilisateur avec l'email {email} existe déjà. "
                        f"Le membre a été créé sans nouveau compte."
                    )
                else:
                    ...
                    try:
                        ...
                        message = render_to_string('membres/email_new_member.html', context_mail)
                        mail = EmailMessage(
                        ...
                        messages.success(
                            request,
                            f"✅ Membre ajouté avec succès. Un email de bienvenue a été envoyé à {email}."
                        )

                    except Exception as e:
                        messages.warning(
                            request,
                            f"Membre ajouté, mais l'email n'a pas pu être envoyé : {e}"
                        )
            else:
                # Aucun email fourni → membre créé sans compte
                messages.success(
                    request,
                    "✅ Membre ajouté avec succès (aucun email fourni)."
                )
...
def member_delete(request, member_id):
    ...
    if request.method == "POST":
        ...
        # Supprimer le compte User lié si existant
        if member.user:
            member.user.delete()  # supprime aussi le member en cascade si OneToOne
        else:
            member.delete()
